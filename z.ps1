param(
    [string]$DumpDirectory = "C:\Users\andrew\Desktop\B850"
)
...
$commands = @"
    ...
    # BIOS version and mobo revision
    !sysinfo machineid
    
    # processor info
    !sysinfo cpuinfo

    # Basic crash analysis - verbose output with stack, bugcheck params, etc.
    !analyze -v

    # Current thread and its stack trace
    !thread
                                          
    # Virtual memory overview + commit / paged / non-paged pool stats
    !vm

    # Top 200 pool tags by usage (good for spotting pool leaks/corruption)
    !poolused 0 200

    # Loaded modules (drivers) with file timestamps + descriptions
    lmof

    # Kernel critical section / mutex / spinlock contention
    !locks

    # Driver Verifier global settings
    !verifier

    # Verifier tracked pool allocations (very useful for corruption)
    !verifier 3

    # Minifilter drivers loaded in the system
    !fltkd.filters

    # Network adapters and their state (useful for NDIS-related crashes)
    !ndiskd.netadapter

    # All open handles in the system (warning: can be very slow / large output)
    !handle 0 0

    # Summary of all processes at crash time
    !process 0 0

    # Detailed memory usage breakdown (can produce huge output)
    !memusage

    # ────────────────────────────────────────────────
    # End of commands - close log and quit
    .logclose
    q
"@
